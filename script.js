...
const calcDisplayBalance = (acc) => {
    let movements = acc.movements;
    acc.balance = movements.reduce((acc, curr) => acc + curr, 0);
...
const calcDisplaySummary = (acc) => {
    ...
    const interest = movements.filter(mov => mov > 0).map(deposit => (deposit * (acc.interestRate)) / 100).reduce((acc, curr) => acc + curr, 0)
...
btnClose.addEventListener('click', (e) => {
    ...
    const delUserPin = Number(inputClosePin.value);
    if (delUserName === currentAccount.username && delUserPin === currentAccount.pin) {
